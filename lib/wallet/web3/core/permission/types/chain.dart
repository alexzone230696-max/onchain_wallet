...
import 'package:on_chain_wallet/wallet/web3/networks/aptos/permission/models/permission.dart';
...
typedef Web3ChainNetwork<NETWORKADDRESS> = Web3Chain<NETWORKADDRESS, Chain,
    ChainAccount, Web3ChainAccount, WalletNetwork>;

abstract class Web3Chain<
    NETWORKADDRESS,
    CHAIN extends Chain,
    ACCOUNT extends ChainAccount,
    CHAINACCOUT extends Web3ChainAccount,
  ...
  factory Web3Chain.deserialize(
    ...
    if (chain
        is! Web3Chain<NETWORKADDRESS, CHAIN, ACCOUNT, CHAINACCOUT, NETWORK>) {
  ...
  List<ACCOUNT> getAccountsPermission(
      {required List<CHAINACCOUT> accounts, required CHAIN chain}) {
    return accounts
        .map((e) => _getAccountPermission(account: e, chain: chain))
        .toList();
  }
