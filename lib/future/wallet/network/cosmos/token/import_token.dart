...
import 'package:on_chain_wallet/future/wallet/network/cosmos/account/state.dart';
    ...
    return NetworkAccountControllerView<CosmosClient, ICosmosAddress,
      ...
      childBulder: (wallet, account, client, address, onAccountChanged) {
        return _CosmosImportTokenView(
            apiProvider: client, account: account, wallet: wallet);
      },
    );
  }
}
...
class __CosmosImportTokenViewState
  ...
  void fetchTokens() async {
    ...
    final result = await MethodUtils.call(() async {
      return await widget.apiProvider.getAddressTokens(address);
    });
    if (result.hasError) {
  ...
  Future<void> add(CW20Token token) async {
    await MethodUtils.call(() async =>
        await widget.account.addNewToken(token: token, address: address));
  }

  Future<void> removeToken(CW20Token token) async {
    await MethodUtils.call(() async =>
        await widget.account.removeToken(token: token, address: address));
  }
