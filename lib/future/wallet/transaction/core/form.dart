...
mixin TransactionStatePageController<SUCCESS extends SubmitTransactionSuccess,
    SIGNEDTX, CHAINTRANSACTION> on DisposableMixin {
  ...
  Widget onTxCompleteWidget(
      SIGNEDTX signedTx, CHAINTRANSACTION transaction, SUCCESS txId) {
    return SuccessTransactionTextView(
        network: network, txIds: [txId.txId], error: txId.warning);
  }

  void setTxComplete(
      SIGNEDTX signedTx, CHAINTRANSACTION transaction, SUCCESS txId) {
    pageKey.success(
        progressWidget: onTxCompleteWidget(signedTx, transaction, txId),
        backToIdle: false);
...
abstract class TransactionStateController<
    ...
    with
        ...
        TransactionStatePageController<SUCCESS, SIGNEDTX, T> {
  ...
  bool get allowSwitchAddress => true;
  ...
  Future<void> signAndSendTransaction({BuildContext? context}) async {
    ...
    setTxComplete(result.result.$2, currentTx.transaction, successResult);
  }
